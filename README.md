# I3T Digital & Analog Clock

## How to run it

* Click the **[INPUT - Play]** button (Pulse) to start the clock.
* To pause the clock, click **[INPUT - Pause]**, and to reset it, click **[INPUT - Stop & Reset]**.
* You can also change the starting time under **[INPUT - Hours/Minutes/Seconds]**.
* To change the speed of the clock, adjust the **[INPUT - Step size per second]**.

---

## How it works

### 1. Main View
![Main View](screenshots/1-main.png)

### 2. Modulos Logic
![Modulos](screenshots/2-modulos.png)

### 3. Blinking Colons
![Colons](screenshots/3-colons.png)

### 4. Digit Bits Map
![Digit Bits](screenshots/4-digit-bits.png)

### 5. Final Digit Rendering
![Digit Rendering](screenshots/5-digit-rendering.png)

### 6. Day/Night Cycle (Sun & Moon)
![Sun Moon](screenshots/6-sun-moon.png)

### 7. Analog Clock: Digits
![Analog Clock Digits](screenshots/7-analog-clock-digits.png)

### 8. Analog Clock: Lines
![Analog Lines](screenshots/8-analog-lines.png)

### 9. Analog Clock: Hands
![Analog Hands](screenshots/9-analog-hands.png)